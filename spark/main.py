...
if __name__ == "__main__":
    ...
    MONGO_SPARK_PACKAGE = "org.mongodb.spark:mongo-spark-connector_2.12:3.0.0"
    ...
    questions_df = questions_df \
        .withColumn("OwnerUserId", F.col("OwnerUserId").cast("integer")) \
    ...
    extract_languages_udf = F.udf(extract_programming_languages, returnType=ArrayType(StringType()))
    ...
    programming_language_df = questions_df \
        .withColumn("extract_languages", extract_languages_udf("Title")) \
    ...
    domain_df = questions_df \
        .withColumn("extract_domain", extract_domains_udf("Body")) \
    ...
    date_df = questions_df \
        ...
        .where((F.col("CreationDate").between(start_date, end_date)) & (F.col("OwnerUserId").isNotNull())) \
    ...
    questions_df.coalesce(1).write \
        ...
        .format("parquet") \
        .mode("overwrite") \
        .option("path", "{}/question_data".format(DATA_DIR)) \
        .saveAsTable("NEW_DB.question_df2")
        
    answers_df.coalesce(1).write \
        .bucketBy(10, "ParentId") \
        .format("parquet") \
        .mode("overwrite") \
        .option("path", "{}/answer_data".format(DATA_DIR)) \
        .saveAsTable("NEW_DB.answer_df2")
